...
export const useTouchToSelectionBridge = ({
  ...
  const bridgeTouchToSelection = useCallback((
    ...
    debugLog('TouchToSelectionBridge', `Bridge attempt for ${action}`, {
      ...
      toolIsSelect2: effectiveTool === 'select2',
      isReadOnly,
      touchCount,
      totalTouches,
    ...
    // Bridge for select2 tool only, single-finger touches
    if (effectiveTool !== 'select2' || isReadOnly) {
      debugLog('TouchToSelectionBridge', 'Bridge conditions not met', {
        currentTool,
        effectiveTool,
        toolNotSelect2: effectiveTool !== 'select2',
        isReadOnly,
        touchCount,
        totalTouches
      });
