...
export const calculateGroupBounds = (
  ...
  try {
    ...
    for (const image of selectedImages) {
      ...
      // For rotated images, check all corners using pooled points
      const corners = [
        { x: 0, y: 0 },
        { x: width, y: 0 },
        { x: width, y: height },
      ...
      for (const corner of corners) {
        // Use pooled point for rotated corner calculation
        const rotatedCorner = pointPool.acquire();
        borrowedPoints.push(rotatedCorner);
        
        // Rotate
        rotatedCorner.x = corner.x * cosRotation - corner.y * sinRotation;
        rotatedCorner.y = corner.x * sinRotation + corner.y * cosRotation;
        
        // Translate
        const x = rotatedCorner.x + image.x;
        const y = rotatedCorner.y + image.y;
    ...
    // Use pooled bounds object for final result, but return a new object
    // since this will be stored in state
    return {
      x: minX - padding,
      y: minY - padding,
    ...
    // Release all borrowed points back to the pool
    for (const point of borrowedPoints) {
